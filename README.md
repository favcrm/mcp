...
138 typed tools — customers, bookings, loyalty, invoices, payments, WhatsApp / SMS / email — exposed via MCP. Works with any agentic client that speaks Streamable HTTP transport.
...
Restart Cursor → `Settings → MCP → favcrm` connects → 138 tools land in chat.
...
138 tools across 22 scopes. Every tool ships with annotations:
